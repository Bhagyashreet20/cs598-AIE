...
def training_function(config, args):
    ...
    train_dataset = datasets["train"].select(random.sample(range(len(datasets["train"])), 4000))
    val_dataset = datasets["validation"].select(random.sample(range(len(datasets["validation"])), 1000))
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_id)  # Example with BART
    ...
    #     )
    def collate_fn(examples):
    # Set max_length for inputs and targets
        max_length = 512 if accelerator.distributed_type == DistributedType.XLA else None
        target_max_length = 128

        # Pad inputs and targets separately
    ...
    model = AutoModelForSeq2SeqLM.from_pretrained(model_id)  # Update to your desired model   
