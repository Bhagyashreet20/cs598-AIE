...
class Trainer:
    ...
    def __init__(
        ...
        self.inner_train_loop_count=0
        self._save_checkpoint_count=0
        if args is None:
    ...
    def _tune_save_checkpoint(self, checkpoint_dir: str):
        ...
        if self.args.should_save:
            ...
            # dcp.async_save(self.optimizer.state_dict(), checkpoint_id =os.path.join(output_dir, OPTIMIZER_NAME))
            # dcp.async_save(self.lr_scheduler.state_dict(), checkpoint_id =os.path.join(output_dir, SCHEDULER_NAME))
    ...
    def _inner_training_loop(
        ...
        # print(f" Time: {current_time}, Inner training loop count: {self.inner_train_loop_count}, inner training loop start")
        ...
        # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # print(f"Time: {current_time}, Inner training loop count: {self.inner_train_loop_count}, inner training loop ends")
        self.inner_train_loop_count+=1
    ...
    def _transfer_model_state_dict(self, model):
        # global rank, device
        if self.rank == 1: # send 
            dst_rank = self.rank-1
            model_state_dict_cpu = model.state_dict()
            model_state_dict = {k: v.to(self.device) for k, v in model_state_dict_cpu.items()}
            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
            print(f"{current_time} Rank {self.rank}: start sending!! model_state_dict: type {type(model_state_dict)}, length: {len(model_state_dict)}")
            # first, send a num
            tensor_to_send = torch.tensor(len(model_state_dict), dtype=torch.int64).to(self.device)
            # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
            # print(f"0---{current_time} Rank {self.rank}: Send length of dict {len(model_state_dict)} with size {tensor_to_send.element_size()*tensor_to_send.numel()} to Rank {dst_rank}, device: {tensor_to_send.device}")
            dist.send(tensor_to_send, dst=dst_rank)
            # Serialize the state_dict as tensors
            for key, tensor in model_state_dict.items():
                    # Send tensor key and data
                    to_send_tensor1 = torch.tensor(len(key), dtype=torch.int64).to(self.device)
                    # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                    # print(f"1---{current_time} Rank {self.rank}: Sending tensor of size {to_send_tensor1.element_size()*to_send_tensor1.numel()} to Rank {dst_rank}")
                    dist.send(to_send_tensor1, dst=dst_rank)
                    
                    byte_array_key = bytearray(key, 'utf-8')
                    to_send_tensor2 = torch.tensor(list(byte_array_key), dtype=torch.uint8).to(self.device)
                    # print(f"{type(to_send_tensor2)}, dtype:, size: {len(to_send_tensor2)}")
                    # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                    # print(f"2---{current_time} Rank {self.rank}: Sending tensor of size {to_send_tensor2.element_size()*to_send_tensor2.numel()} to Rank {dst_rank}")
                    dist.send(to_send_tensor2, dst=dst_rank)

                    ndim_tensor = torch.tensor(len(tensor.size()), dtype=torch.int64, device=self.device)
                    # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                    # print(f"3---{current_time} Rank {self.rank}: Sending tensor of size {ndim_tensor.element_size()*ndim_tensor.numel()} to Rank {dst_rank}")
                    dist.send(ndim_tensor, dst=dst_rank)

                    shape_tensor = torch.tensor(tensor.size(), dtype=torch.int64, device=self.device)
                    # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                    # print(f"4---{current_time} Rank {self.rank}: Sending tensor of size {shape_tensor.element_size()*shape_tensor.numel()} to Rank {dst_rank}")
                    ...
                    # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                    # print(f"5---{current_time} Rank {self.rank}: Sending tensor of size {tensor.element_size()*tensor.numel()} to Rank {dst_rank}")
                    dist.send(tensor, dst=dst_rank)

            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
            print(f"{current_time} Rank {self.rank} done sending-------------------------")

        elif self.rank==0: # receive
            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
            print(f"{current_time} rank: {self.rank},start receiving")
            src_rank = self.rank+1
            # 1.first, get how many key_value pairs this rank is gonna receive:
            # Receive tensor key
            num_length_tensor = torch.empty(1, dtype=torch.int64).to(self.device)
            dist.recv(num_length_tensor, src=src_rank)
            number_of_dict_members = num_length_tensor.item()
            # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
            # print(f"0---{current_time} rank: {self.rank},received num_length_tensor {number_of_dict_members}")
            received_state_dict = {}
            for i in range(number_of_dict_members):
                # Receive tensor key
                
                key_length = torch.empty(1, dtype=torch.int64).to(self.device)
                dist.recv(key_length, src=src_rank)
                # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                # print(f"1---{current_time} Rank {self.rank}: received key_length. Size: {key_length.element_size()*key_length.numel()}. And the received key length is {key_length.item()}")

                key = torch.empty(key_length.item(), dtype=torch.uint8).to(self.device)
                dist.recv(key, src=src_rank)
                key_cpu = key.cpu().numpy().tobytes().decode('utf-8')
                current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                # print(f"2---{current_time} Rank {self.rank}: received key. Size: {key.element_size()*key.numel()}, received key is {key_cpu}")
                # key = key.cpu().numpy().tobytes().decode('utf-8')
                # print(f"Rank {rank}: received key is {key}")

                len_of_shape = torch.empty(1, dtype=torch.int64, device=self.device)
                dist.recv(len_of_shape, src=src_rank)
                # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                # print(f"3---{current_time} Rank {self.rank}: Received len_of_shape. Size: {len_of_shape.element_size()*len_of_shape.numel()}")

                ndim = len_of_shape.item()
                shape_tensor = torch.empty((ndim,), dtype=torch.int64, device=self.device)
                dist.recv(shape_tensor, src=src_rank)
                # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                # print(f"4---{current_time} Rank {self.rank}: Received shape_tensor. Size: {shape_tensor.element_size()*shape_tensor.numel()}")

                shape = tuple(shape_tensor.tolist())   
                tensor = torch.empty(shape, dtype=torch.float32, device=self.device)
                dist.recv(tensor, src=src_rank)
                # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                # print(f"5---{current_time} Rank {self.rank}: Received tensor. Size: {tensor.element_size()*tensor.numel()}")
                received_state_dict[key_cpu] = tensor.cpu()

            model.load_state_dict(received_state_dict)
            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
            print(f"{current_time} Rank {self.rank} done receiving-------------------------")

    def _maybe_log_save_evaluate(self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
        print(f"device: {self.rank}, Time: {current_time}, naybelogsaveevaluate start")
        ...
        # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # print(f"device: {self.rank}, Time: {current_time}, barrier start")
        # dist.barrier(group=self.transfer_group)
        # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # print(f"device: {self.rank}, Time: {current_time}, barrier end")
        self._transfer_model_state_dict(model)
        if self.rank == 0 and self.control.should_save:
            # rank = int(os.getenv('LOCAL_RANK'))
            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            print(f"device: {self.rank}, Time: {current_time}, _save_checkpoint count: {self._save_checkpoint_count}, _save_checkpoint start")
            self._save_checkpoint(model, trial, metrics=metrics)
            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            print(f"device: {self.rank}, Time: {current_time}, _save_checkpoint count: {self._save_checkpoint_count}, _save_checkpoint end")
            self._save_checkpoint_count+=1
            self.control = self.callback_handler.on_save(self.args, self.state, self.control)

        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
        print(f"device: {self.rank}, Time: {current_time}, naybelogsaveevaluate end")
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # print(f"device: {self.rank}, Time: {current_time}, in _save_checkpoint,  checkpoint_folder")
        if self.hp_search_backend is None and trial is None:
            # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            # print(f"device: {self.rank}, Time: {current_time}, in _save_checkpoint,  store_flos")
            self.store_flos() # should do nothing seriously
        # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # print(f"device: {self.rank}, Time: {current_time}, in _save_checkpoint,  _get_output_dir")
        run_dir = self._get_output_dir(trial=trial)
        output_dir = os.path.join(run_dir, checkpoint_folder)
        self.save_model(output_dir, _internal_call=True)
        # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # print(f"device: {self.rank}, Time: {current_time}, in _save_checkpoint,  done save_model")
        if not self.args.save_only_model:
            # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            # print(f"device: {self.rank}, Time: {current_time}, in _save_checkpoint,  not self.args.save_only_model")
            # Save optimizer and scheduler
            self._save_optimizer_and_scheduler(output_dir)
            # Save RNG state
            self._save_rng_state(output_dir)
            # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            # print(f"device: {self.rank}, Time: {current_time}, in _save_checkpoint,  saved optimizer and scheduler and rngstate")

        # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # print(f"device: {self.rank}, Time: {current_time}, in _save_checkpoint, metrics is not None and self.args.metric_for_best_model is not None??????")
        ...
        # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # print(f"device: {self.rank}, Time: {current_time}, in _save_checkpoint, should_save? save the trainer state?")
        ...
        # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # print(f"device: {self.rank}, Time: {current_time}, in _save_checkpoint,  push_to_hub?")
        if self.args.push_to_hub:
            # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            # print(f"device: {self.rank}, Time: {current_time}, in _save_checkpoint, push_to_hub!")
        ...
        # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # print(f"device: {self.rank}, Time: {current_time}, in _save_checkpoint, should save? Delete some older checkpoints?")
        ...
        # current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # print(f"device: {self.rank}, Time: {current_time}, in _save_checkpoint, All done")

    def _save_rng_state(self, output_dir): # random number generator
        ...
        if self.args.world_size <= 1:
            ...
            # dcp.async_save(rng_states, checkpoint_id = os.path.join(output_dir, "rng_state.pth"))
        else:
            torch.save(rng_states, os.path.join(output_dir, f"rng_state_{self.args.process_index}.pth"))
            # dcp.async_save(rng_states, checkpoint_id = os.path.join(output_dir, f"rng_state_{self.args.process_index}.pth"))
    ...
    def save_model(self, output_dir: Optional[str] = None, _internal_call: bool = False):
        ...
        """
        # print(f"rank {self.rank}: save_model")
        if output_dir is None:
            output_dir = self.args.output_dir

        if is_torch_xla_available():
            # print(f"rank {self.rank}: is_torch_xla_available")
            self._save_tpu(output_dir)
        elif is_sagemaker_mp_enabled():
            # print(f"rank {self.rank}: is_sagemaker_mp_enabled")
            ...
            # print(f"rank {self.rank}: is_fsdp_enabled")
            ...
            # print(f"rank {self.rank}: deepspeedenabled")
            try:
            ...
            # print(f"rank {self.rank}: should_save 666")
            self._save(output_dir)

        # Push to the Hub when `save_model` is called by the user.
        if self.args.push_to_hub and not _internal_call:
            # print(f"rank {self.rank}: push_to_hub")
    ...
    def _save_tpu(self, output_dir: Optional[str] = None):
        ...
        if xm.is_master_ordinal(local=False):
            ...
            # dcp.async_save(self.args, checkpoint_id =os.path.join(output_dir, TRAINING_ARGS_NAME))
        ...
        if not isinstance(self.model, supported_classes):
            ...
            if isinstance(self.accelerator.unwrap_model(self.model), supported_classes):
                ...
                if self.args.save_safetensors:
                    ...
                    # dcp.async_save(state_dict, checkpoint_id =os.path.join(output_dir, WEIGHTS_NAME))
        else:
