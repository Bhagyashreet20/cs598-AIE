...
accelerator = Accelerator(
    ...
    fsdp_config={
        ...
        "cpu_offload": True,  # Enables parameter and gradient offloading to CPU
    },
)
...
# Wrap model with FSDP for efficient sharding
model = FSDP(model)

# Load and preprocess the dataset
dataset = load_dataset("wikitext", "wikitext-2-raw-v1")  # Use Wikitext-2 for fine-tuning
def tokenize_function(examples):
    # Tokenize and truncate to a maximum length
...
training_args = TrainingArguments(
    ...
    weight_decay=0.01,
    logging_dir="./logs",
    report_to="none",  # Disable logging to avoid unnecessary output
)
...
for epoch in range(training_args.num_train_epochs):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        # Backward pass with FSDP and optimizer step
