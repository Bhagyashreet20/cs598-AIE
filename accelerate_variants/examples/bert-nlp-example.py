...
def training_function(config, args):
    ...
    print(f"Initial model embedding size: {model.get_input_embeddings().weight.size()}")
    ...
    # prepare method.
    print(f"Embedding shape before accelerator prepare: {model.get_input_embeddings().weight.size()}")
    model, optimizer, train_dataloader, eval_dataloader, lr_scheduler = accelerator.prepare(
        model, optimizer, train_dataloader, eval_dataloader, lr_scheduler
    )
    print(f"Embedding shape after accelerator prepare: {model.get_input_embeddings().weight.size()}")
    ...
    for epoch in range(starting_epoch, num_epochs):
        model.train()
        print(f"Embedding weight shape after resizing: {model.get_input_embeddings().weight.size()}")
        if args.with_tracking:
            total_loss = 0
        ...
        for step, batch in enumerate(eval_dataloader):
            # We could avoid this line since we set the accelerator with `device_placement=True`.
            print(f"Embedding weight shape in eval: {model.get_input_embeddings().weight.size()}")
            batch.to(accelerator.device)
            with torch.no_grad():
                outputs = model(**batch)
